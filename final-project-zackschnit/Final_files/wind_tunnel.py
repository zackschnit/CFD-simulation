...
import time
import tunnel_calculations as tc
import unpack_jpg as jpg
import sys
...
#two pre programed shaped
if file_name == "square.jpg":
    obj[15:25, 35:45] = 0
    
elif file_name == "circle.jpg":
    ...
    try:
        obj = jpg.read_file("images/" + file_name, nx, ny)
    except:
...
def flow(u, v, p):
    ...
    for itts in range(nt):
        ...
        opperation_rate = (itts + 1) / time_elapsed
        
        time_remaining = (nt - itts) / opperation_rate
        
        if (itts % 50 == 0):
            print(f"Estimated Time Remaining: {round(time_remaining)} seconds")
...
def plotFlow(u, v, p, **kwargs):
    ...
    windSpeedList, forceList, area = tc.windAndForce(u, v, p, obj, dx)
    windSpeedList = np.array(windSpeedList)
    forceList = np.array(forceList)
    
    global cutoff
    cutoff, windSpeedListCut, others = tc.cleanUpLists(windSpeedList, 1, forceList)
    ...
    def update(frame):
        ...
        plt.suptitle(f"Wind Tunnel Simulation for {file_name}", fontweight = "bold", fontsize = 18)
        
        tunnelPlot = 111
        if data:
            tunnelPlot = 211
            plt.subplot(212)
            plt.title("SIMULATION DATA", fontweight = 'bold', fontsize = 14)
            plt.axis('off')
            params = tc.dragCalculations(windSpeedList, forceList, rho, area)
            drag = params[1]
            plt.text(0.5, 0.9, f"Wind Speed: {round(windSpeedList[frame],2)} m/s", ha='center', va='center', fontsize = 14)
            plt.text(0.5, 0.5, f"Characteristic Length: {round(area,4)} m", ha='center', va='center', fontsize = 14)
            if drag == -1:
                plt.text(0.5, 0.7, "Cannot Calculate Drag", color = "C1", fontweight = 'bold', ha='center', va='center', fontsize = 14)
            else:
                plt.text(0.5, 0.7, f"Drag Coefficient: {round(drag,2)}", ha='center', va='center', fontsize = 14)
            plt.text(0.5, 0.3, f"Density: {rho} kg/m^3", ha='center', va='center', fontsize = 14)
        ...
        if pressure:
            ...
            plt.imshow(p[frame], alpha=0.5, cmap="viridis", extent = (np.min(x), np.max(x), np.max(y), np.min(y)))
        ...
        plt.imshow(obj, cmap=object_cmap, extent = (np.min(x), np.max(x), np.max(y), np.min(y)))
...
#call the function
changeParameters(timeItterations = 300, force = 10000, viscosity = 0.00002, timeStep = 0.00001)
uSol, vSol, pSol = flow(u_initial, v_initial, p_initial)

anim = plotFlow(uSol, vSol, pSol, data = True, vectors = True)
